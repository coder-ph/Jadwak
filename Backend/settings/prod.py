...
# jenga_platform/settings/prod.py
...
# Production database (read from environment variable, as setup in base.py)
# DATABASES already set up to read from DATABASE_URL env var in base.py.
# Ensure DATABASE_URL is correctly set in your production environment.

# Security settings
CSRF_COOKIE_SECURE = True
SESSION_COOKIE_SECURE = True
SECURE_SSL_REDIRECT = env.bool('DJANGO_SECURE_SSL_REDIRECT', default=True) # Ensure HTTPS
SECURE_HSTS_SECONDS = env.int('DJANGO_SECURE_HSTS_SECONDS', default=31536000) # 1 year
SECURE_HSTS_INCLUDE_SUBDOMAINS = True
SECURE_HSTS_PRELOAD = True
SECURE_PROXY_SSL_HEADER = ('HTTP_X_FORWARDED_PROTO', 'https') # Required if behind Nginx/load balancer

# CORS settings for production (restrict to your frontend domain)
CORS_ALLOW_ALL_ORIGINS = False
CORS_ALLOWED_ORIGINS = env.list('DJANGO_CORS_ALLOWED_ORIGINS', default=['https://yourfrontend.com'])
...
# AWS_S3_CUSTOM_DOMAIN = env('AWS_S3_CUSTOM_DOMAIN', default=f'{AWS_STORAGE_BUCKET_NAME}.s3.amazonaws.com') # Optional, for CDN
...
CELERY_BROKER_URL = env('CELERY_BROKER_URL_PROD', default='redis://your_prod_redis_host:6379/1') # Example: use DB 1
CELERY_RESULT_BACKEND = env('CELERY_RESULT_BACKEND_PROD', default='redis://your_prod_redis_host:6379/1')
